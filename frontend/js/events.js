...
async function onModalScanSuccess(decodedText, decodedResult) {
    ...
    // Process the QR code
    document.getElementById('scanStatus').innerHTML = `
        <span style="color: #3b82f6;">📱 QR Code detected! Processing...</span>
    `;
        ...
        if (data.success) {
            document.getElementById('scanStatus').innerHTML = `
                <div style="color: #10b981; font-weight: 600; font-size: 1.1rem;">
                    ✓ ${data.message}
                    <br><span style="font-size: 0.9rem;">Event: ${data.event_name}</span>
                    <br><span style="font-size: 0.85rem; color: #6b7280;">Student: ${data.student_name}</span>
                </div>
            `;
...
async function stopModalScanning() {
    // Try to stop scanner if it exists, regardless of flag state
    if (html5QrCodeModal) {
        try {
            ...
            // Scanner wasn't running, just ensure flag is false
            isModalScanning = false;
            console.log('Scanner already stopped or not started');
        }
    }
    
    // Reset modal display and status
    document.getElementById('qrScannerModal').style.display = 'none';
    document.getElementById('scanStatus').textContent = 'Position QR code within frame';
    document.getElementById('scanStatus').style.color = '#374151';
}
